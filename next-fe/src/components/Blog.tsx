...
export const Blog = ({ image, title, slug, body }: Props) => {
  const capitalizedTitle = _.startCase(_.toLower(title))
  const content = body.slice(0, 160) + ' ...'
  return (
    <Link href={`/blogs/${slug}`} className='shadow-sm bg-white border min-h-60 h-auto rounded-xl overflow-hidden flex flex-col md:flex-row'>
      <Thumbnail
        className='h-32 md:h-full object-cover object-center min-w-[none] md:min-w-7 w-full md:w-1/3'
        image={image}
      />
      <div className='flex p flex-col w-2/3 p-4 gap-4'>
